...
def add_polygons(func, box: cube, polygons: Mesh):
    case = 0
    vertices = box.get_vertices()

    for i, p in enumerate(vertices):
        value = func(p.x, p.y, p.z)
        if value > 0:
            case += pow(2, i)
...
def _marching_cubes(func, box: cube, precision: int, cubes: list[cube], polygons: Mesh):
    if (box.delta() <= precision):
        add_polygons(func, box, polygons)
        return
    ...
    if (isBorder):
        boxes = box.divide()
        for i in range(8):
            _marching_cubes(func, boxes[i], precision, cubes, polygons)
...
def draw_curve(func, output_file: str, min_x:float, min_y:float, min_z:float, max_x: float, max_y: float, max_z: float, precision: float):
    ...
    _marching_cubes(func, box, precision, cubes, polygons)
